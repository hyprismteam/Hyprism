...
`WizardHost` обрабатывает Esc в открытом мастере: выводит фокус из текстового поля, отменяет зарегистрированную операцию, возвращает на предыдущий шаг или закрывает мастер. Экраны регистрируют команды переходов и закрытия. Когда компоновка переходит через порог компактного режима, экран синхронизирует хост с текущим состоянием мастера, чтобы завершить прерванный переход.
...
`FadingPopup` сохраняет явно заданное положение. Всплывающие окна с `PlacementGap` выбирают положение над целью или под ней в зависимости от доступного места.
...
| `ModalTextField` | [`ModalTextField.cs`](../../../../Sources/Hyprism.Desktop/UI/Controls/Primitives/ModalTextField.cs), слот `TextBox`, `IsError` и `ErrorMessage`; используется в формах настроек и редакторах профилей и экземпляров | Обычное состояние, наведение, фокус, многострочный ввод и ошибка проверки с подсказкой у значка | [Форма аргументов Java](../../../static/img/user/ru/java-arguments.png) |
...
Для ошибки валидации одной строки используйте `FormRow.Error`. `ModalForm` задает общий макет формы внутри `OverlayModal`; редакторы профиля и экземпляра собирают собственные панели. Необязательный `BackdropTarget` размывает и блокирует основной экран. Модалки настроек не задают его и используют только затемнение оверлея. `InitialFocusTarget` задает первое поле. При закрытии восстанавливаются прежний фокус и эффект фона, если он был задан.

`OverlayModal` рисует нижнюю линию открытой панели и управляет нижней обводкой родительской рамки `mainSceneFrame`, пока видна хотя бы одна панель. Рамка возвращает нижнюю обводку после завершения последней анимации закрытия. Для новых модалок не нужно регистрировать это состояние в модели окна.
