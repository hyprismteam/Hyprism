...
Official profiles always use official authentication and do not show the Auth Server editor. For offline profiles, the server list appears only when Connected mode is enabled. The built-in `sessions.sanasol.ws` entry is always listed. `AuthServers` stores additional user-added entries, while `AuthDomain` stores the selected value. Desktop checks each listed server through the game-session authentication API, but does not store the availability result. Local Node chooses a free port from `8443 to 9999` on `127.0.0.1`. Neither the address nor the per-launch port is stored in this file.
...
Some persisted properties are kept for compatibility and may not have a dedicated editor in the current UI.
...
| `HYPRISM_WIN32_COMPOSITION` | Windows rendering diagnostic override |
