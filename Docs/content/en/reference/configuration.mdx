...
| `InstanceDirectory` | empty | Default instance root, or the `HyprismLibrary` path created inside the directory selected in Desktop |
...
A persisted property does not imply a completed Desktop workflow. The current onboarding flow remains unfinished

Older configurations can contain a custom instance root outside `HyprismLibrary`. Desktop recognizes those paths when reading configuration. When moving from such a path, it copies recognized instance data and leaves the old directory in place. Desktop marks libraries it creates with `.hyprism-library` and only removes marked custom roots after a successful move.
