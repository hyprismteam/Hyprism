...
Desktop instance action
  ...
  -> GameProcessTracker and per-instance logs
```
...
`GameVersionCatalog` selects a mirror and resolves its request headers for the selected download URL. `GameInstallationWorkflow` and `PatchManager` pass those headers to `HttpFileDownloader` for file size checks and transfers. Mirror discovery and availability checks use the same launcher version placeholders. See [mirror definitions](/reference/mirrors) for discovery methods and header fields.
