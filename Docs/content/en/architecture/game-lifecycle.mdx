...
`GameInstallationWorkflow` rejects concurrent installation work for the same instance. Each instance has an explicit branch and numeric version. The workflow records `PendingVersion` before changing an installation, which allows an interrupted update to resume safely. When an installed instance already matches the selected version, the workflow can use a fast path without contacting version sources or mirrors.
