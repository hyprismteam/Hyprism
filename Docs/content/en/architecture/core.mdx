...
CurseForge file relations are stored in the instance manifest. Required relations are resolved recursively before installation. Cycles and unresolved required nodes fail the operation. Optional relations are recorded but are not installed automatically. Incompatible relations are checked against the current installed set. JAR and ZIP archives are inspected for a root `manifest.json`, whose plugin identifier, version, and dependency ranges are stored for validation and removal protection.
