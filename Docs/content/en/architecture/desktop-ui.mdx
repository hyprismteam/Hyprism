...
| `ModalTextField` | [`ModalTextField.cs`](../../../../Sources/Hyprism.Desktop/UI/Controls/Primitives/ModalTextField.cs), a `TextBox` content slot, `IsError`, and `ErrorMessage` | Rest, hover, focus, and validation error with a warning tooltip | [Java argument form](../../../static/img/user/en/java-arguments.png) |
...
Use `FormRow.Error` for a validation message that belongs to one row. Use `ModalForm` for a form inside `OverlayModal`; `BackdropTarget` owns the blur and input block, while `InitialFocusTarget` names the first input. Closing the modal restores the backdrop effect and the element that had focus before opening.
