...
The Desktop project keeps `Hyprism.Desktop` as its assembly identity for Avalonia resource URIs. Its build and publish targets rename only the native apphost to `Hyprism Launcher`, so the executable name stays consistent without breaking embedded UI resources
