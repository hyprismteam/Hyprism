...
Open the [local documentation](http://localhost:3000/launcher/docs/). Development and GitHub Pages use the same `/launcher/docs` base path.
...
`DocsSidebar.tsx` renders the category tree in the left column on wide screens. At compact widths, `Navbar/MobileSidebar/index.tsx` renders the search field and the same tree in a full-viewport panel below the navbar. The panel closes on outside click, Escape, or navigation and does not render a second header.
...
`npm run check` validates prose, language parity, code fences, local images, internal routes, explicit bilingual anchors, repository links, and Core contract documentation, then runs TypeScript checks. The static export goes to `Docs/out/launcher/docs` so the published site is available at `/launcher/docs/`.
...
The published entry point is the [Hyprism documentation site](https://hyprism.org/launcher/docs/). Do not commit `out`, `.docusaurus`, `node_modules`, or obsolete `.next` build products.
