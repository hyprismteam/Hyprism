...
The run refreshes the selection control sheet, instance creation wizard, Java settings, and Java argument dialog images used by the [desktop UI guide](/architecture/desktop-ui/).
...
`npm run check` validates prose, language parity, code fences, local images, internal routes, explicit bilingual anchors, repository links, and Core contract documentation, then runs TypeScript checks. The static export goes to `Docs/out/launcher/docs` so GitHub Pages serves it at `/launcher/docs/`.
